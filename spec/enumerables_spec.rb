...
describe '#get_spicy_food_by_cuisine' do
  it 'returns a single hash for the spicy food whose cuisine matches the cuisine being passed to the method' do
    expect(get_spicy_food_by_cuisine(spicy_foods,
                                     'Sichuan')).to eq({ name: 'Mapo Tofu', cuisine: 'Sichuan', heat_level: 6 })
    expect(get_spicy_food_by_cuisine(spicy_foods,
                                     'Thai')).to eq({ name: 'Green Curry', cuisine: 'Thai', heat_level: 9 })
  end
end
